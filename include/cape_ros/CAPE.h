...
class CAPE {
 ...
 public:
  CAPE(int depth_height, int depth_width, int cell_width, int cell_height,
       bool cylinder_detection, float min_cos_angle_4_merge = 0.97814,
       float max_merge_dist = 900);
  void process(Eigen::MatrixXf& cloud_array, int& nr_planes, int& nr_cylinders,
               cv::Mat& seg_output, vector<PlaneSeg>& plane_segments_final,
               vector<CylinderSeg>& cylinder_segments_final,
               std::vector<std::vector<cv::Point> >& polygons);
  void RegionGrowing(unsigned short width, unsigned short height, bool* input,
                     bool* output, vector<PlaneSeg*>& Grid,
                     vector<float>& cell_dist_tols, unsigned short x,
                     unsigned short y, double* normal, double d);
