...
CAPE::CAPE(int depth_height, int depth_width, int cell_width, int cell_height,
           bool cylinder_detection, float min_cos_angle_4_merge,
  ...
  distances_stacked =
      (float *)malloc(depth_height * depth_width * sizeof(float));
  distances_cell_stacked = Eigen::ArrayXf::Zero(nr_pts_per_cell, 1);
  seg_map_stacked = (unsigned char *)malloc(depth_height * depth_width *
...
void CAPE::process(Eigen::MatrixXf &cloud_array, int &nr_planes_final,
                   int &nr_cylinders_final, cv::Mat &seg_out,
  ...
  std::memset(seg_map_stacked, (uchar)0,
              depth_height * depth_width * sizeof(unsigned char));
  std::memset(
      distances_stacked, 100,
      depth_height * depth_width * sizeof(float)); /* = to really high float*/
  ...
  for (int cell_r = 0; cell_r < nr_vertical_cells; cell_r++) {
    for (int cell_c = 0; cell_c < nr_horizontal_cells; cell_c++) {
      Grid[stacked_cell_id] = new PlaneSeg(cloud_array, stacked_cell_id,
                                           nr_pts_per_cell, cell_width);
      if (Grid[stacked_cell_id]->planar) {
        cell_diameter =
            (cloud_array.block(
                 stacked_cell_id * nr_pts_per_cell + nr_pts_per_cell - 1, 0, 1,
                 3) -
             cloud_array.block(stacked_cell_id * nr_pts_per_cell, 0, 1, 3))
                .norm();
        // Add truncated distance
        cell_distance_tols[stacked_cell_id] =
            pow(min(max(cell_diameter * sin_cos_angle_4_merge, 20.0f),
                    max_merge_dist),
                2);
      }
  ...
  while (nr_remaining_planar_cells > 0) {
    ...
    RegionGrowing(nr_horizontal_cells, nr_vertical_cells, unassigned_mask,
                  activation_map, Grid, cell_distance_tols, x, y, seed_n,
                  seed_d);

    // 3. Merge activated cells & remove them from histogram and list of
    // remaining cells
    ...
    for (int cell_r = 0; cell_r < nr_vertical_cells; cell_r++) {
      ...
      for (int cell_c = 0; cell_c < nr_horizontal_cells; cell_c++) {
        ...
        if (row_ptr[cell_c] > 0) {
          ...
          distances_cell_stacked =
              cloud_array.block(offset, 0, nr_pts_per_cell, 1).array() * nx +
  ...
  if (cylinder_detection) {
    ...
    for (int i = 0; i < nr_cylinders; i++) {
      ...
      grid_cylinder_seg_map_eroded.setTo(
          (unsigned char)cylinder_code_offset + nr_cylinders_final,
          mask_eroded > 0);
            ...
            for (int pt = offset; pt < next_offset; j++, pt++) {
              ...
              if (point(2) > 0) {
                ...
                if (dist < max_dist && dist < distances_stacked[pt]) {
                  distances_stacked[pt] = dist;
                  seg_map_stacked[pt] =
                      cylinder_code_offset + nr_cylinders_final;
                }
              }
            }
          }
  ...
  for (int cell_r = 0; cell_r < nr_vertical_cells; cell_r++) {
    ...
    grid_cylinder_eroded_row_ptr =
        grid_cylinder_seg_map_eroded.ptr<uchar>(cell_r);
    ...
    for (int cell_c = 0; cell_c < nr_horizontal_cells; cell_c++) {
      ...
      if (grid_plane_eroded_row_ptr[cell_c] > 0) {
        ...
        if (grid_cylinder_eroded_row_ptr[cell_c] > 0) {
          ...
          stack_ptr =
              &seg_map_stacked[nr_pts_per_cell * cell_r * nr_horizontal_cells +
  ...
  for (int i = 0; i < nr_cylinders; i++) {
    ...
    if (reg_id > -1) {
      ...
      copy(cylinder_segments[reg_id].axis, cylinder_segments[reg_id].axis + 3,
           cy.axis);
...
void CAPE::getConnectedComponents(cv::Mat &segment_map,
  ...
  for (int r = 0; r < planes_association_matrix.rows(); r++) {
    for (int c = r + 1; c < planes_association_matrix.cols(); c++) {
      planes_association_matrix(r, c) =
          planes_association_matrix(r, c) || planes_association_matrix(c, r);
    }
  }
}
...
void CAPE::RegionGrowing(unsigned short width, unsigned short height,
                         bool *input, bool *output, vector<PlaneSeg *> &Grid,
                         vector<float> &cell_dist_tols, unsigned short x,
                         unsigned short y, double *normal_1, double d) {
  int index = x + width * y;
  ...
  if (x > 0)
    RegionGrowing(width, height, input, output, Grid, cell_dist_tols, x - 1, y,
                  normal_2, d_2);  // left  pixel
  if (x < width - 1)
    RegionGrowing(width, height, input, output, Grid, cell_dist_tols, x + 1, y,
                  normal_2, d_2);  // right pixel
  if (y > 0)
    RegionGrowing(width, height, input, output, Grid, cell_dist_tols, x, y - 1,
                  normal_2, d_2);  // upper pixel
  if (y < height - 1)
    RegionGrowing(width, height, input, output, Grid, cell_dist_tols, x, y + 1,
                  normal_2, d_2);  // lower pixel
}
