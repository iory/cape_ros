 ...
 *
 */
#include "cape_ros/CylinderSeg.h"

CylinderSeg::CylinderSeg(vector<PlaneSeg*>& Grid, bool* activated_mask,
                         int nr_cells_activated) {
  ...
  while (m_left > 5 && m_left > 0.1 * m) {
    ...
    while (k < K) {
      ...
      A = e1.transpose() * e1;
      a = 1 - A(0) / 9;
      b = (N.col(id_1).array() * P_proj.col(id_1).array() +
           N.col(id_2).array() * P_proj.col(id_2).array() +
      ...
      // Normal dist
      D = ((P_proj - r * N).colwise() - center.col(0)).colwise().squaredNorm() /
          (r * r);
...
float CylinderSeg::distance(Eigen::Vector3f& P3, int id) {
  return ((P2[id] - P1[id]).cross(P3 - P2[id])).norm() / P1P2_norm[id] -
         radii[id];
}
