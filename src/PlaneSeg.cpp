...
#include "cape_ros/PlaneSeg.h"

PlaneSeg::PlaneSeg(Eigen::MatrixXf& cloud_array, int cell_id,
  ...
  int j = i + cell_width;
  float z(0),
      z_last(max(Z_matrix(i),
                 Z_matrix(i + 1))); /* handles missing pixels on the borders*/
  i++;
  ...
  z_last =
      max(Z_matrix(i),
          Z_matrix(i + cell_width)); /* handles missing pixels on the borders*/
  i = i + cell_width;
  jumps_counter = 0;
  while (i < j) {
    z = Z_matrix(i);
  ...
  if (planar) {
    fitPlane();
    if (MSE >
        pow(DEPTH_SIGMA_COEFF * mean[2] * mean[2] + DEPTH_SIGMA_MARGIN, 2)) {
      planar = false;
    }
  }
}
...
void PlaneSeg::fitPlane() {
  ...
  double cov[3][3] = {
      {xx_acc - x_acc * x_acc / nr_pts, xy_acc - x_acc * y_acc / nr_pts,
       xz_acc - x_acc * z_acc / nr_pts},
  ...
  Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> es(
      Eigen::Map<Eigen::Matrix3d>(cov[0], 3, 3));
